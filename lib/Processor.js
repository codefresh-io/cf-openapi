...
class Processor {
    ...
    postprocess(spec, options = {}) {
        ...
        // _.set(spec, 'servers[0].url', CF_API_URL); // todo: add url

        return isRaw ? spec : specProcessors.reduce((spec, processor) => processor(spec), spec);
    }
    ...
    _resolveSpecTagsCollisions(spec, serviceId) {
        _.forEach(spec.tags, (tag) => tag['x-id'] = `${serviceId}:${tag['x-id']}`);
    }
    ...
    _resolveComponentsCollisions(spec, serviceId) {
        ...
        _.forEach(spec.components, (category, categoryName) => {
            _.forEach(category, (component, componentName) => {
                const fixedComponentName = `${serviceId}__${componentName}`;
