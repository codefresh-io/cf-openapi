...
const servicesNames = require('@codefresh-io/internal-service-config').names;
...
class SpecProvider extends Base {
    ...
    internalServicesNames() {
        return this.internalServices().map(s => servicesNames[s]);
    }

    validateInternalServices() {
        const invalidServices = this.internalServices().filter(s => !servicesNames[s]);
