...
class Endpoints extends Base {
    constructor() {
        ...
        this.missingScopeHandler = missingScopes => new Error(`Scopes are missing to access this resource: ${missingScopes}`);
    }
    ...
    register(app) {
        ...
        this.registerScopesEndpoint(app);
    ...
    registerScopesEndpoint(app) {
        if (this.spec === false) {
            console.log('Scope endpoint is disabled');
            return;
        }
        ...
        app.get(scopesPath, ...middlewares, (req, res) => {
            res.send(this.components.spec.collectScopes());
        });
    }
    ...
    /**
     * @param extractorFn - function that receives request and returns user scope array
     * */
            ...
            _.forEach(resource, (method, httpMethod) => {
                ...
                const {
                    auth: {
                        ...
                        'access-control': accessControlOptions,
                    } = {},
                    preMiddleware = [],
                    postMiddleware = [],
                    handler,
                ...
                const authPreMiddleware = _.map(authMiddleware, metaPath => this._resolveComponent(this.middleware, metaPath));

                const preparedAccessControlOptions = this._prepareAccessControlOptions(accessControlOptions, urlPath, httpMethod);
                const scopePreMiddleware = this._createScopeMiddleware(preparedAccessControlOptions);
                const abacPreMiddleware = this._createAbacMiddleware(preparedAccessControlOptions);

                const logicPreMiddleware = _.map(preMiddleware, metaPath => this._resolveComponent(this.middleware, metaPath));

                const endpointPreMiddleware = _.concat(authPreMiddleware, scopePreMiddleware, abacPreMiddleware, logicPreMiddleware);
    ...
    _createScopeMiddleware(accessControlOptions) {
        ...
        return [(request, response, next) => {
            const userScope = self.scopeExtractor(request);
            if (userScope === null) {
                next();
                return;
            }

            const endpointScope = self._resolveEndpointScope(accessControlOptions);
    ...
    _createAbacMiddleware(preparedAccessControlOptions) {
        ...
        if (options.disableAbac) {
            return [];
        }

        const abacFactory = this.abac[options.resource];
        if (!abacFactory) {
            return [];
        }
    ...
    _hasEnoughScope(userScope, endpointScope) {
        const [endpointResource, endpointAction] = endpointScope.split(':');
        let hasEnoughScope = false;
        _.forEach(userScope, (scope) => {
            const [scopeResource, scopeAction] = scope.split(':');
            if (scopeResource === endpointResource) {
                if (scopeAction === SCOPE_ACTIONS.admin) {
                    hasEnoughScope = true;
                }
                if (scopeAction === SCOPE_ACTIONS.write && [SCOPE_ACTIONS.read, SCOPE_ACTIONS.write].includes(endpointAction)) {
                    hasEnoughScope = true;
                }
                if (scopeAction === endpointAction) {
                    hasEnoughScope = true;
                }
            }
