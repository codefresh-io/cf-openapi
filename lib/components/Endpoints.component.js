...
class Endpoints extends Base {
    ...
    registerRedocEndpoints(app) {
        ...
        if (this.isDocsEndpointDisabled()) {
            console.log(`Skip exposing api docs endpoint: API_DOCS_ENDPOINT_DISABLED = ${this.isDocsEndpointDisabled()}`);
            return;
        }
    ...
    isDocsEndpointDisabled() {
        return _.get(this, 'config.openapi.docsEndpointDisabled', false);
    }
    ...
    createGeneralScopeMiddleware(endpoint = {}) {
        ...
        const endpointGeneralScopes = _.values(this.components.spec.GENERAL_SCOPES);
