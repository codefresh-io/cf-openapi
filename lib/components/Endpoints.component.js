...
class Endpoints extends Base {
    ...
    validateAppEndpoints() {
        ...
        const errors = [];
        _.forEach(spec.paths, (resource, path) => {
            _.forEach(resource, (method, methodName) => {
                const httpMethod = _.toUpper(methodName);
                const endpoint = method['x-endpoint'];
                if (!endpoint) {
                    errors.push({
                        type: 'endpoint',
                        message: `x-endpoint property is missing for path: ${httpMethod} ${path}`,
                    });
                    return;
                }
                if (!endpoint.handler) {
                    errors.push({
                        type: 'endpoint',
                        message: `x-endpoint.handler property is missing for path: ${httpMethod} ${path}`,
                    });
                } else {
                    const [namespace, operation] = endpoint.handler.split('.');
                    this._validateComponent('controller', namespace, operation, errors, httpMethod, path);
                }

                _.forEach(_.union(endpoint.preMiddleware, endpoint.postMiddleware), (middleware) => {
                    const [namespace, operation] = middleware.split('.');
                    this._validateComponent('middleware', namespace, operation, errors, httpMethod, path);
                });

                if (endpoint.condition) {
                    const [namespace, operation] = endpoint.condition.split('.');
                    this._validateComponent('condition', namespace, operation, errors, httpMethod, path);
                }
            });
        });
        if (!_.isEmpty(errors)) {
            const message = _.chain(errors)
                .reduce((acc, err) => {
                    ...
                    acc[err.type].push(err.message);
                    return acc;
                }, {})
                .mapValues(messages => `\n\t${messages.join('\n\t')}`)
                .mapKeys(type => `${type} errors`)
                .reduce((acc, messages, type) => _.union(acc, [`${type}${messages}`]))
                .join('\n')
                .value();
            throw new Error(`Validation failed for openapi endpoints:\n${message}`);
        }
    }

    _validateComponent(component, namespace, operation, errors, httpMethod, path) {
        if (!_.has(this, `${component}.${namespace}`)) {
            errors.push({
                type: component,
                message: `No such controller: ${namespace}.controller.js. Path: ${httpMethod} ${path}`,
            });
        } else if (!_.has(this, `${component}.${namespace}.${operation}`)) {
            errors.push({
                type: component,
                message: `${namespace}.${component}.js has no such method: ${operation}(). Path: ${httpMethod} ${path}`,
            });
        }
    }
