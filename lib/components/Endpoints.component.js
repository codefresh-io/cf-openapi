...
class Endpoints extends Base {
    ...
    validateAppEndpoints() {
        ...
        _.forEach(spec.paths, (resource, url) => {
            _.forEach(resource, (method, methodName) => {
                ...
                const abacSource = _.get(endpoint, 'auth.acl.abac');
                if (abacSource && !this.abac[abacSource]) {
                    errors.push({
                        component: 'acl.abac',
                        message: `${abacSource}.abac.js does not exist`,
                        spec_method: methodName,
                        spec_url: url,
                    });
                }
    ...
    _createAbacMiddleware(preparedAccessControlOptions) {
        ...
        const source = options.abac || options.resource;
    ...
    _hasEnoughScope(userScopeArr, endpointScope) {
        ...
        _.forEach(userScopeArr, (userScope) => {
            hasEnoughScope =
                userScope === `${endpointResource}:${SCOPE_ACTIONS.admin}`
                || (endpointAction !== SCOPE_ACTIONS.admin && endpointScope.startsWith(userScope));
