...
class Endpoints extends Base {
    ...
    _preparePreMiddleware(endpointMeta) {
        ...
        if (!_.isEmpty(abacPreMiddleware)) {
            const preparedAbacOptions = this._prepareAbacOptions(preparedAclOptions);
    ...
    validateAppEndpoints() {
        ...
        _.forEach(spec.paths, (resource, url) => {
            _.forEach(resource, (method, methodName) => {
                ...
                const preparedAclOptions = this._prepareAccessControlOptions(aclOptions, url, methodName);
                const preparedAbacOptions = this._prepareAbacOptions(preparedAclOptions);
                const scopes = this.components.spec.collectScopeArray();
                const scope = this._resolveEndpointScope(preparedAclOptions);
                ...
                if (aclOptions.abac) {
                    _.forEach(preparedAbacOptions, (option) => {
                        ...
                        if (!this.abac[abacSource || aclResource]) {
                            errors.push({
                ...
                if (this.scopeExtractor && !preparedAclOptions.disableScopes) {
                    const nestingComponents = scope.split(':');
                    let checkedParent = '';
                    _.forEach(nestingComponents, (component) => {
                        checkedParent = checkedParent ? `${checkedParent}:${component}` : component;
                        if (!scopes.includes(checkedParent)) {
                            errors.push({
                                component: specFilename,
                                message: `"${scope}" should have parent scope "${checkedParent}"`,
                                spec_method: methodName,
                                spec_url: url,
                            });
                        }
    ...
    _createScopeMiddleware(accessControlOptions) {
        ...
        return [async (request, response, next) => {
            ...
            try {
                userScope = await self.scopeExtractor(request, [endpointScope]);
            } catch (e) {
                next(e);
                return;
            }
    ...
    _createAbacMiddleware(preparedAccessControlOptions) {
        ...
        const abacMiddleware = abacFactory(this._prepareAbacOptions(options));
    ...
    _prepareAccessControlOptions(options, urlPath, httpMethod) {
        ...
        return _.merge(options, {
            resource, action, urlPath, httpMethod,
        });
    }

    _prepareAbacOptions(preparedAclOptions) {
        const { abacOptions = [{}] } = preparedAclOptions;
        const optionsProto = _.cloneDeep(preparedAclOptions);
        _.unset(optionsProto, 'abacOptions')
        return _.map(abacOptions, option => _.defaultsDeep(option, optionsProto));
    }
    ...
    _resolveEndpointScope(preparedAccessControlOptions) {
        const { resource, action, scope } = preparedAccessControlOptions; // eslint-disable-line
        const calculatedScope = scope || this._resolveScopeFromAction(action);
