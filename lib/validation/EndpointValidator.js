...
class EndpointValidator {
    ...
    validate(endpoint, options = {}) {
        ...
        if (!endpoint.spec.handler) {
            ...
            this._validateAppComponent('controller', endpoint.spec.handler, endpoint);
        }
        ...
        _.forEach(middlewareToValidate, (middleware) => {
            this._validateAppComponent('middleware', middleware, endpoint);
        });

        if (endpoint.spec.condition) {
            this._validateAppComponent('condition', endpoint.spec.condition, endpoint);
        }
        ...
        if (options.isScopeExtractorEnabled && !endpoint.spec.acl.disableScopes) {
