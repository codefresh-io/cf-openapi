...
function formatUrl(spec) {
    const pathParams = _.chain(spec.parameters)
        .filter(p => p.in === 'path')
        .map(p => p.name);
    const path = pathParams.reduce((url, param) => {
        return url.replace(`{${param}}`, `${formatEnvVar(param)}`);
    }, spec.path);
...
function generateCurl(spec) {
    ...
    let curlStr = `curl ${data.join('')}${_formatNewLine(`"${formatUrl(spec)}${formatParams(spec)}"`)}`;
    return {
        path: spec.path,
        method: spec.method,
        curl: [curlStr],
...
function generateCurlSamples(spec) {
    ...
    let paths = _.cloneDeep(spec.paths);
    return _.chain(paths)
        .map((resource, path) => {
            return _.map(resource, (method, methodName) => {
        ...
        .flatten()
        .map((method) => _.map(method.security, (security) => _.merge(method, { security })))
        .flatten()
        .map(generateCurl)
        .reduce((acc, obj) => {
            const path = `${obj.path}.${obj.method}`;
            const res = acc[path] || { path, curl: [] };
...
function addCurlSamples(spec) {
    ...
    _.forEach(samples, (sample, path) => {
        path = `${path}.x-code-samples`;
        _.set(_spec.paths, path, _.get(_spec.paths, path, []).concat([{
            lang: 'curl',
